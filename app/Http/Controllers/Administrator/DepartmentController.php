...
class DepartmentController extends Controller
{
    ...
    {
        $campuses = Campus::all();
        return view('admin.department.index', compact('campuses'));
    }
    ...
    public function fetch()
    {
        ...
        if ($departments->count() > 0) {
            $output = '<div class="table-responsive">
                        <table class="table align-middle table-row-bordered table-row-dashed gy-5 all_departments_table" id="kt_table_widget_1">
                            <thead>
                                <tr class="text-start text-gray-400 fw-boldest fs-7 text-uppercase">
                                    ...
                                    <th class="min-w-200px px-0">Department Name</th>
            ...
            foreach ($departments as $department) {
                $output .= '<tr>
                                ...
                                <!-- Department Name -->
                                <td class="p-0">
                                    <span class="text-gray-800 fs-5">' . ucfirst($department->name) . '</span>
                                </td>
                                <!-- Campus Name -->
                                <td class="p-0">
                                    <span class="text-gray-800 fs-5">' . ucfirst($department->campus->name) . '</span>
                                </td>

                                <!-- Created At -->
                                <td>
                                    <span class="text-gray-800 fs-5 d-block">' . $department->created_at->format('M d, Y') . '</span>
                                </td>
